...
async function loadFieldCards(kb: KbConfig): Promise<FieldCardView[]> {
  ...
  for (const meta of (await listCards(kb)).filter((card) => card.type === 'field')) {
    ...
    views.push({ meta, fm: parseFrontmatter(card.raw).frontmatter ?? {}, body: card.body })
  }
  return views
}

function identifiersOf(view: FieldCardView): string[] {
  const raw = [view.meta.slug, view.meta.title, view.fm.canonical_name, view.fm.field_id]
...
function mentions(body: string, identifier: string): boolean {
...
function evidenceAround(body: string, identifier: string): string {
...
function pushProposal(list: LineageProposal[], proposal: LineageProposal): void {
  ...
  for (const key of RELATION_KEYS) {
    ...
    existing.relations[key] = [...new Set([...(existing.relations[key] ?? []), ...incoming])]
  }
}
 ...
 */
export async function scanLineage(kb: KbConfig): Promise<LineageScanResult> {
  const cards = await loadFieldCards(kb)
  const proposals: LineageProposal[] = []
  const notes: string[] = []

  // Identifier → slug index (slug wins over title/alias collisions).
  const byIdentifier = new Map<string, string>()
  for (const card of cards) {
    for (const identifier of identifiersOf(card)) {
      const key = normalize(identifier)
      if (!byIdentifier.has(key)) byIdentifier.set(key, card.meta.slug)
    }
  }
  const resolve = (value: string): string | null => byIdentifier.get(normalize(value)) ?? null

  for (const card of cards) {
    const selfIdentifiers = new Set(identifiersOf(card).map(normalize))

    // 1. Body mentions of other field cards → candidate dependency (direction
    //    is always "this card depends on the mentioned card").
    ...
    // 2. Existing dangling depends_on → surface as a note for this card.
    for (const target of asStringList(card.fm.depends_on)) {
      if (resolve(target) === null) {
        notes.push(`${card.meta.title}: depends_on「${target}」尚无对应字段卡（lint 会报 relation-dangling），建议建卡后再登记`)
      }
    }
    ...
    if (sourceTable !== '' && NON_TABLE_SOURCES.test(sourceTable)) {
      const implemented = [...new Set([...asStringList(card.fm.implemented_in), sourceTable])]
      pushProposal(proposals, {
        slug: card.meta.slug,
        title: card.meta.title,
        source: 'scan',
    ...
    if (card.meta.description === undefined || card.meta.description.trim() === '') {
      notes.push(`${card.meta.title}: 缺一句话摘要（面版卡片墙无法展示），建议补充 description`)
    }
    if (card.fm.field_kind === 'dimension' && card.fm.aggregation === 'additive') {
      pushProposal(proposals, {
        slug: card.meta.slug,
        title: card.meta.title,
        source: 'scan',
    ...
    if (card.fm.evidence_level === undefined) {
      pushProposal(proposals, {
        slug: card.meta.slug,
        title: card.meta.title,
        source: 'scan',
  ...
  return { kb: kb.id, generatedAt: new Date().toISOString(), fieldCards: cards.length, proposals, notes }
}

/** Path of the parked agent-proposal file for one KB. */
export function lineageProposalFile(kb: KbConfig): string {
  return join(configRoot(), 'lineage', `${kb.id}.json`)
}

export async function readLineageProposals(kb: KbConfig): Promise<LineageProposalFile | null> {
  try {
    const text = await fs.readFile(lineageProposalFile(kb), 'utf8')
    const parsed = JSON.parse(text) as LineageProposalFile
...
export async function writeLineageProposals(
  kb: KbConfig,
  proposals: LineageProposal[],
  extra: { requestedAt?: string; childId?: string; notes?: string[] } = {},
): Promise<LineageProposalFile> {
  const file: LineageProposalFile = {
    kb: kb.id,
    requestedAt: extra.requestedAt ?? new Date().toISOString(),
 ...
 */
export function buildLineagePrompt(kb: KbConfig, cards: FieldCardMeta[]): string {
...
export async function listFieldCardMeta(kb: KbConfig): Promise<FieldCardMeta[]> {
  return (await loadFieldCards(kb)).map((view) => ({ slug: view.meta.slug, title: view.meta.title }))
}
 ...
 */
export async function applyLineage(kb: KbConfig, accepted: LineageProposal[]): Promise<LineageApplyResult> {
  const applied: LineageApplyResult['applied'] = []
  const skipped: LineageApplyResult['skipped'] = []
  const bySlug = new Map<string, LineageProposal[]>()
  ...
  for (const [slug, proposals] of bySlug) {
    ...
    for (const proposal of proposals) {
      for (const key of RELATION_KEYS) {
        const incoming = proposal.relations[key]
        if (incoming === undefined) continue
        next[key] = [...new Set([...asStringList(next[key]), ...incoming])]
      }
    ...
    // Provenance: never downgrade an owner-confirmed card; otherwise mark the
    //助 assistant-written content as inferred.
    const reviewStatus = typeof current.review_status === 'string' ? current.review_status : ''
    ...
    applied.push({ slug, title: result.card.title, changed: result.changed, relations })
  }

  return { kb: kb.id, applied, skipped, generatedAt: new Date().toISOString() }
}
