...
/** Card shape as declared by the wiki_search output schema (no path). */
...
async function resolveKb(kbId: string | undefined): Promise<{ id: string; name: string } | null> {
  const kbs = await listKbSummaries()
  if (kbs.length === 0) return null
  if (kbId !== undefined && kbId !== '') {
    const hit = kbs.find((kb) => kb.id === kbId)
    return hit !== undefined ? { id: hit.id, name: hit.name } : null
  }
  const env = process.env.DSH_KNOWLEDGE_CARDS_KB
  if (env !== undefined && env !== '') {
    const hit = kbs.find((kb) => kb.id === env)
    if (hit !== undefined) return { id: hit.id, name: hit.name }
  }
  return { id: kbs[0].id, name: kbs[0].name }
}

function renderCardSummary(card: { slug: string; type: string; title: string; description?: string; tags: string[]; sources: string[]; related: string[] }): string {
  ...
  return `- [${card.type}] ${card.slug} — ${card.title}${card.description !== undefined ? `\n  ${card.description}` : ''}${tags}${sources}${related}`
}

export function wikiKbsTool() {
  return defineTool({
    name: 'wiki_kbs',
    ...
    output: {
      ...
      render: (_args, value: { kbs: Array<{ id: string; name: string; path: string; total: number; sourceCount: number }> }) => {
    ...
    async execute() {
      const summaries = await listKbSummaries()
      return {
        kbs: summaries.map((summary) => ({
          id: summary.id,
          name: summary.name,
          path: summary.path,
...
export function wikiSearchTool() {
  return defineTool({
    name: 'wiki_search',
    ...
    parameters: {
      query: { type: 'string', required: true, description: '检索词（支持中文与英文，标题/描述/标签/正文加权匹配）。' },
      kb: { type: 'string', description: '知识库 id（wiki_kbs 查看；省略用默认库）。' },
    ...
    output: {
          ...
          cards: {
            ...
            items: {
              ...
              properties: {
                ...
                related: { type: 'array', items: { type: 'string' }, required: true },
      ...
      render: (_args, value: { kb: string; total: number; cards: SearchHitCard[] }) => {
    ...
    async execute(args: { query: string; kb?: string; limit?: number }) {
...
export function wikiReadTool() {
  return defineTool({
    ...
    parameters: {
      slug: { type: 'string', required: true, description: '卡片 slug（文件名 stem 或标题）。' },
    ...
    output: {
      ...
      render: (_args, value: { kb: string; slug: string; card: { path: string; type: string; title: string; description?: string; tags: string[]; related: string[]; sources: string[]; raw: string } }) => {
        return text(`[wiki_read] 知识库「${value.kb}」卡片 ${value.slug}（${value.card.path}）:\n\n${value.card.raw}`)
      },
    },
    async execute(args: { slug: string; kb?: string }) {
      const resolved = await resolveKb(args.kb)
...
const SOURCE_CONTENT_CAP = 60_000
const CONTEXT_CAP = 8_000

async function readContextFile(kbPath: string, name: string): Promise<string> {
  try {
    const content = await fs.readFile(join(kbPath, name), 'utf8')
...
export function wikiIngestTool() {
  return defineTool({
    name: 'wiki_ingest',
    ...
    parameters: {
      kb: { type: 'string', description: '知识库 id（省略用默认库）。' },
      source: { type: 'string', description: '可选：raw/sources/ 下的资料相对路径（如 "policy-2024.pdf" 或 "sub/folder.docx"）。省略则只列待摄入清单。' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (_args, value: { kb: string; pending: number; sources: Array<{ relPath: string; status: string; sha256: string }>; sourceContent?: string; context?: string }) => {
        ...
        if (value.sourceContent === undefined) {
          return text(header + '\n\n未指定 source。请先选定一份待摄入资料，或用文件工具把资料放入 raw/sources/ 后重试。')
        }
        return text(header + '\n\n=== 资料全文（截断）===\n' + value.sourceContent + '\n\n=== 知识库上下文 ===\n' + (value.context ?? ''))
      },
    },
    async execute(args: { kb?: string; source?: string }) {
      const resolved = await resolveKb(args.kb)
      if (resolved === null) throw new Error('尚无知识库。先创建知识库。')
      const kb = await getKb(resolved.id)
      if (kb === null) throw new Error(`unknown knowledge base: ${resolved.id}`)
      ...
      if (sourceArg !== undefined && sourceArg !== '') {
        const match = sources.find((source) => source.relPath === sourceArg || source.relPath.endsWith(sourceArg))
        if (match === undefined) {
          throw new Error(`raw/sources/ 下找不到 "${sourceArg}"。可用资料: ${sources.map((source) => source.relPath).join(', ') || '（空）'}`)
        }
        const raw = await fs.readFile(join(kb.path, 'raw', 'sources', match.relPath), 'utf8')
...
export function wikiCommitTool() {
  return defineTool({
    name: 'wiki_commit',
    ...
    parameters: {
      ...
      sourceFiles: { type: 'array', items: { type: 'string' }, description: '本次摄入涉及的 raw/sources/ 资料相对路径，用于标记增量缓存。' },
    },
    output: {
      schema: {
        type: 'object',
        ...
        properties: {
          ...
          cachedSources: { type: 'array', items: { type: 'string' }, required: true },
        },
      },
      render: (_args, value: { kb: string; created: string[]; updated: string[]; logEntry: string; cachedSources: string[] }) => {
    ...
    async execute(args: { kb?: string; pages: PageInput[]; sourceFiles?: string[] }) {
      const resolved = await resolveKb(args.kb)
      if (resolved === null) throw new Error('尚无知识库。先创建知识库。')
      const kb = await getKb(resolved.id)
      if (kb === null) throw new Error(`unknown knowledge base: ${resolved.id}`)
      if (!Array.isArray(args.pages) || args.pages.length === 0) {
        throw new Error('pages 不能为空：至少提供一张 {type,title,body} 卡片。')
      }
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          warns: { type: 'integer', required: true },
        },
      },
      render: (_args, value: { kb: string; report: string }) => text(value.report),
    },
...
export function wikiCreateKbTool() {
  return defineTool({
    name: 'wiki_create_kb',
    ...
    parameters: {
      name: { type: 'string', required: true, description: '知识库名称（如 "财务政策库"）。' },
      path: { type: 'string', description: '可选：知识库目录绝对路径；省略用默认根。' },
    ...
    output: {
      ...
      render: (_args, value: { kb: { id: string; name: string; path: string; total: number } }) => {
    ...
    async execute(args: { name: string; path?: string; description?: string }) {
      const kb = await createKb({ name: args.name, path: args.path, description: args.description })
      const summary = await kbSummary(kb)
      return {
        kb: { id: kb.id, name: kb.name, path: kb.path, total: summary.stats.total },
      }
    },
  })
}

export function wikiImportCardsTool() {
  return defineTool({
    ...
    parameters: {
      kb: { type: 'string', description: '知识库 id（省略用默认库）。' },
      dir: { type: 'string', required: true, description: '本地目录绝对路径，内含要导入的 .md 卡片文件（递归扫描）。' },
    },
    output: {
      schema: {
        type: 'object',
        ...
        properties: {
          kb: { type: 'string', required: true },
          imported: { type: 'array', items: { type: 'string' }, required: true },
          skipped: {
            type: 'array',
            required: true,
            items: {
          ...
          sourceFiles: { type: 'array', items: { type: 'string' }, required: true },
        },
      },
      render: (_args, value: { kb: string; imported: string[]; skipped: Array<{ file: string; reason: string }>; sourceFiles: string[] }) => {
    ...
    async execute(args: { kb?: string; dir: string }) {
      const resolved = await resolveKb(args.kb)
...
const CODE_READ_CAP = 60_000

export function wikiCodeListTool() {
  return defineTool({
    name: 'wiki_code_list',
    ...
    output: {
      ...
      render: (_args, value: { kb: string; files: Array<{ relPath: string; size: number }> }) => {
    ...
    async execute(args: { kb?: string }) {
      ...
      return {
        kb: resolved.id,
        files: files.map((file) => ({ relPath: file.relPath, size: file.size, mtime: file.mtime })),
      }
    },
  })
}

export function wikiCodeReadTool() {
  return defineTool({
    name: 'wiki_code_read',
    ...
    parameters: {
      kb: { type: 'string', description: '知识库 id（省略用默认库）。' },
      path: { type: 'string', required: true, description: 'code/ 下的相对路径（wiki_code_list 查看）。' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (_args, value: { kb: string; path: string; content: string; truncated: boolean }) => {
        return text(`[wiki_code_read] 知识库「${value.kb}」代码 ${value.path}:\n\n${value.content}${value.truncated ? '\n…[内容已截断]' : ''}`)
      },
    },
    async execute(args: { kb?: string; path: string }) {
      const resolved = await resolveKb(args.kb)
      if (resolved === null) throw new Error('尚无知识库。先 wiki_create_kb 建库。')
      const kb = await getKb(resolved.id)
      if (kb === null) throw new Error(`unknown knowledge base: ${resolved.id}`)
      ...
      return { kb: resolved.id, path: args.path, content, truncated }
    },
  })
}
...
export function wikiEditCardTool() {
  return defineTool({
    ...
    parameters: {
      ...
      title: { type: 'string', description: '新标题（留空不改）。' },
      description: { type: 'string', description: '新一句话摘要（空串 = 清除）。' },
      tags: { type: 'array', items: { type: 'string' }, description: '新标签列表（提供即整体替换）。' },
...
export function wikiReviewSubmitTool() {
  return defineTool({
    ...
    parameters: {
      kb: { type: 'string', description: '知识库 id（省略用默认库）。' },
      kind: { type: 'string', required: true, description: 'contradiction | duplicate | missing-page | suggestion' },
      title: { type: 'string', required: true, description: '简短主题（涉及的实体/概念）。' },
      summary: { type: 'string', required: true, description: '为什么需要人工判断。' },
      source: { type: 'string', description: '触发该审核的资料或卡片。' },
      options: { type: 'array', items: { type: 'string' }, description: '预定义操作（省略按 kind 默认）。' },
      searchQuery: { type: 'string', description: '预生成搜索查询（深度研究用）。' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (_args, value: { kb: string; item: { id: string; kind: string; title: string; status: string; options: string[] } }) => {
    ...
    async execute(args: { kb?: string; kind: string; title: string; summary: string; source?: string; options?: string[]; searchQuery?: string }) {
      const resolved = await resolveKb(args.kb)
      if (resolved === null) throw new Error('尚无知识库。先 wiki_create_kb 建库。')
      const kb = await getKb(resolved.id)
      if (kb === null) throw new Error(`unknown knowledge base: ${resolved.id}`)
      const kinds = ['contradiction', 'duplicate', 'missing-page', 'suggestion']
      ...
      return { kb: resolved.id, item: { id: item.id, kind: item.kind, title: item.title, status: item.status, options: item.options } }
    },
  })
}

export function wikiReviewsTool() {
  return defineTool({
    name: 'wiki_reviews',
    ...
    parameters: {
      kb: { type: 'string', description: '知识库 id（省略用默认库）。' },
      status: { type: 'string', description: 'all（默认）| pending | resolved | skipped。' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (_args, value: { kb: string; pending: number; items: Array<{ id: string; kind: string; title: string; summary: string; options: string[]; searchQuery?: string; status: string }> }) => {
        ...
        for (const item of value.items) {
          ...
          lines.push(`    操作: ${item.options.join(' / ')}${item.searchQuery !== undefined ? ` · 搜索: ${item.searchQuery}` : ''}`)
        }
        return text(lines.join('\n'))
      },
    },
    async execute(args: { kb?: string; status?: string }) {
      const resolved = await resolveKb(args.kb)
      if (resolved === null) throw new Error('尚无知识库。先 wiki_create_kb 建库。')
      const kb = await getKb(resolved.id)
      if (kb === null) throw new Error(`unknown knowledge base: ${resolved.id}`)
  ...
  return defineTool({
    ...
    output: {
      schema: {
        ...
        properties: {
          kb: { type: 'string', required: true },
          submitted: { type: 'array', required: true, items: { type: 'object', additionalProperties: false, properties: { id: { type: 'string', required: true }, kind: { type: 'string', required: true }, title: { type: 'string', required: true } } } },
          skippedExisting: { type: 'integer', required: true },
          summary: { type: 'object', required: true, additionalProperties: false, properties: { duplicate: { type: 'integer', required: true }, missingPage: { type: 'integer', required: true } } },
          deepAuditPrompt: { type: 'string', required: true },
        },
      },
      render: (_args, value: { kb: string; submitted: Array<{ id: string; kind: string; title: string }>; skippedExisting: number; summary: { duplicate: number; missingPage: number }; deepAuditPrompt: string }) => {
        const lines = [
          `知识库「${value.kb}」确定性审核完成：新增 ${value.submitted.length} 条审核项（重复 ${value.summary.duplicate} · 缺失页面 ${value.summary.missingPage}），跳过已有 pending ${value.skippedExisting} 条。`,
        ]
