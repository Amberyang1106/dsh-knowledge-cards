...
import { slugFromTitle } from '../core/frontmatter.ts'
import type { KbConfig } from '../core/types.ts'
import { addReview, listCards, listReviews, readCard, resolveLinkTargets } from './store.ts'

export interface AuditResult {
  submitted: Array<{ id: string; kind: string; title: string }>
  skippedExisting: number
  summary: { duplicate: number; missingPage: number }
  /** Prompt the user/agent can run for the semantic (LLM) part of the audit. */
  deepAuditPrompt: string
}

function buildDeepAuditPrompt(kb: KbConfig, cards: Array<{ slug: string; type: string; title: string; description?: string }>): string {
...
export async function buildDeepAuditPromptForKb(kb: KbConfig): Promise<string> {
  const cards = await listCards(kb)
  return buildDeepAuditPrompt(kb, cards.map((card) => ({
    slug: card.slug,
    type: card.type,
    title: card.title,
 ...
 */
export async function auditKb(kb: KbConfig): Promise<AuditResult> {
  const cards = await listCards(kb)
  ...
  const submitted: Array<{ id: string; kind: string; title: string }> = []
  let skippedExisting = 0
  const summary = { duplicate: 0, missingPage: 0 }

  const submitOnce = async (item: { kind: 'duplicate' | 'missing-page'; title: string; summary: string; source?: string; searchQuery?: string }): Promise<void> => {
    ...
    pendingKeys.add(key)
    submitted.push({ id: created.id, kind: created.kind, title: created.title })
  }

  // --- duplicates: same normalized title in different cards ---
  const byNormalized = new Map<string, Array<{ slug: string; title: string }>>()
  for (const card of cards) {
    const normalized = slugFromTitle(card.title)
    const list = byNormalized.get(normalized) ?? []
    list.push({ slug: card.slug, title: card.title })
    byNormalized.set(normalized, list)
  }
  for (const [normalized, group] of byNormalized) {
    if (group.length < 2) continue
    // One review item per collision group, referencing all involved cards.
  ...
  // --- missing pages: [[wikilinks]] pointing at cards that do not exist ---
  ...
  return { submitted, skippedExisting, summary, deepAuditPrompt }
}
