...
import { A as rebuildAggregates, D as purgeKb, E as purgeCard, L as writeCodeFile, M as resolveReview, N as restoreCard, O as readCard, P as restoreKb, S as listTrash, T as pendingSources, _ as listKbSummaries, a as createKb, b as listReviews, c as deleteCodeFile, f as getKb, g as listCodeFiles, h as listCards, i as createCard, k as readCodeFile, l as deleteKb, m as kbSummary, n as commitPages, p as importCards, s as deleteCard, t as addReview, u as editCard, x as listSources, y as listLogEntries } from "./store-DFbZQRmj.js";
import { auditKb, buildDeepAuditPromptForKb } from "./host/audit.js";
import { lintKb } from "./host/lint.js";
import { applyLineage, buildLineagePrompt, confirmFieldCards, listFieldCardMeta, listFieldCardScopes, readLineageProposals, scanLineage } from "./host/lineage.js";
import { lineageConfigState, loadLineageConfig, normalizeLineageConfig, saveLineageConfig } from "./host/lineage-config.js";
import { credentialLookup, resolveJevConfig, runJevLineage } from "./host/jev.js";
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (slugs.length === 0) return json(res, {
						ok: false,
						error: "slugs 不能为空"
					}, 400);
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (req.method === "POST") {
						...
						if (issues.length > 0) return json(res, {
							ok: false,
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (config === null) return json(res, {
						ok: false,
						error: "jev-key-missing",
						detail: "未配置 JEV key：写入 DSH 凭据库 ~/.dsh/.credentials.yaml（推荐，优先级最高、改完立即生效、无需重启）或设环境变量 OPENROUTER_API_KEY（默认线路，https://openrouter.ai/keys，$0.042/M 输入）；直连兜底用 TYPESAFE_API_KEY（https://console.typesafe.ai/keys）。插件只读取凭据、不写入。"
					}, 503);
					ok(res, { result: await runJevLineage(kb, config, {
						forceSlugs: asStringArray(body?.forceSlugs).map((slug) => slug.trim()).filter((slug) => slug !== ""),
