		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (typeof listProviders === "function") try {
						const listed = await listProviders.call(subagents);
						if (Array.isArray(listed)) providerNames = listed.map((entry) => typeof entry === "string" ? entry : String(entry?.name ?? "")).filter((name) => name !== "");
					} catch {
						providerNames = null;
					}
					ok(res, {
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (typeof start !== "function") return json(res, {
						ok: false,
					...
					if (provider === "") {
						...
						if (typeof listProviders === "function") try {
							...
							if (Array.isArray(listed) && listed.length > 0) {
								...
								provider = typeof first === "string" ? first : String(first?.name ?? "");
							}
						} catch {
							provider = "";
						}
					...
					const started = await start.call(subagents, {
						provider,
						label: `血缘补齐 · ${kb.id}`,
