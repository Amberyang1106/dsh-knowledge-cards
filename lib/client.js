	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"lineage.jevDone": "JEV 完成：{n} 条候选（{line} · model {model} · {cards} 张卡 · {questions} 个问题 · {requests} 批请求，最大 {payload} 字符）",
			"lineage.jevKeyMissing": "未配置 JEV key：请设置 OPENROUTER_API_KEY（默认线路，openrouter.ai/keys）或 TYPESAFE_API_KEY（直连兜底，console.typesafe.ai/keys），然后重启 dsh web（插件只读环境变量、不落盘）。JEV 分支只发送最小化 state（元数据 + 去代码/去数字的正文摘要）：卡片内容在 state 里各出现一次，问题仅按 slug 引用。",
		...
		const en = {
			...
			"lineage.jevDone": "JEV done: {n} candidates ({line} · model {model} · {cards} cards · {questions} questions · {requests} requests, max {payload} chars)",
			"lineage.jevKeyMissing": "No JEV key: set OPENROUTER_API_KEY (default line, openrouter.ai/keys) or TYPESAFE_API_KEY (direct fallback, console.typesafe.ai/keys), then restart dsh web (the plugin reads env vars only, never persists them). The JEV branch sends a minimized state — each card appears once, questions reference cards by slug.",
