	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"lineage.jevRunning": "正在调用 JEV（TypeSafe System One）…",
			"lineage.jevDone": "JEV 完成：{n} 条候选（model {model} · {cards} 张卡 · {questions} 个问题）",
			"lineage.jevKeyMissing": "未配置 TYPESAFE_API_KEY：请到 console.typesafe.ai/keys 取 key，设置环境变量后重启 dsh web（插件只读环境变量、不落盘）。JEV 分支只发送最小化 state（元数据 + 去代码/去数字的正文摘要）。",
		...
		const en = {
			...
			"lineage.jevRunning": "Calling JEV (TypeSafe System One)…",
			"lineage.jevDone": "JEV done: {n} candidates (model {model} · {cards} cards · {questions} questions)",
			"lineage.jevKeyMissing": "TYPESAFE_API_KEY is not set: get a key at console.typesafe.ai/keys, export it and restart dsh web (the plugin reads the env var only, never persists it). The JEV branch sends a minimized state (metadata + excerpt with code and numbers stripped).",
