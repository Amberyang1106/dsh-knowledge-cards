...
import { configRoot } from "./store.js";
/** Shipped defaults — the only place a default value is defined. */
export const LINEAGE_CONFIG_DEFAULTS = {
    confidenceHigh: 0.8,
...
export function lineageConfigFields() {
    return CONFIG_KEYS.map((key) => {
        const spec = FIELD_SPECS[key];
        return spec.kind === 'number'
            ? { key, kind: 'number', min: spec.min, max: spec.max, integer: spec.integer, default: LINEAGE_CONFIG_DEFAULTS[key] }
            : { key, kind: 'boolean', default: LINEAGE_CONFIG_DEFAULTS[key] };
    });
}
 ...
 */
export function normalizeLineageConfig(input) {
...
export function overriddenKeys(config) {
 ...
 */
export async function loadLineageConfig(kb) {
    ...
    try {
        ...
        return {
            config: { ...LINEAGE_CONFIG_DEFAULTS },
            overridden: [],
            issues: [{ field: '(file)', message: `配置无法解析为 JSON，已回退默认值：${String(error.message ?? error)}` }],
            fingerprint: lineageConfigFingerprint(LINEAGE_CONFIG_DEFAULTS),
            path,
        };
    }
 ...
 */
export async function saveLineageConfig(kb, config) {
    const path = lineageConfigPath(kb);
    await fs.mkdir(join(configRoot(), 'lineage'), { recursive: true });
    const sparse = {};
    for (const key of overriddenKeys(config))
...
export function lineageConfigState(kb, loaded) {
    return {
        kb: kb.id,
        path: loaded.path,
