...
import { isManagedFrontmatterKey, parseFrontmatter, parseYamlPayload, serializePage, slugFromTitle } from "../core/frontmatter.js";
import { extractWikilinks } from "../core/search.js";
import { REVIEW_OPTIONS } from "../core/types.js";
import { TYPE_DIRS } from "../core/types.js";
...
export async function getKb(id) {
    const config = await loadConfig();
    return config.kbs.find((kb) => kb.id === id) ?? null;
}
export function defaultKbId() {
...
export async function ensureKbStructure(kbPath) {
...
export async function createKb(input) {
    ...
    const config = await loadConfig();
    if (config.kbs.some((kb) => kb.id === id))
...
async function readCardFile(kb, relPath) {
    if (WIKI_AGGREGATES.has(basename(relPath)))
        return null;
...
export async function listCards(kb) {
...
export async function readCard(kb, slug) {
    const files = await walkFiles(wikiDir(kb));
    const target = slug.replace(/^wiki\//, '').replace(/\.md$/, '');
    for (const file of files) {
        const stem = basename(file, '.md');
        if (stem !== target && file !== `${target}.md`)
            continue;
        if (WIKI_AGGREGATES.has(basename(file)))
            continue;
        ...
        if (file !== `${target}.md` && stem !== target && titleSlug !== target)
            continue;
...
export async function resolveLinkTargets(kb) {
    const cards = await listCards(kb);
...
async function rebuildIndex(kb) {
    ...
    const lines = ['# 索引', '', '<!-- 由 dsh-knowledge-cards 自动维护，请勿手改。 -->', ''];
...
export async function listLogEntries(kb) {
...
function safeWikiRelPath(raw) {
    ...
    if (!normalized.endsWith('.md'))
        return null;
    if (!/^[A-Za-z0-9_\u4e00-\u9fff\-\/ ]+\.md$/.test(normalized))
        return null;
    return normalized;
}
/** Write agent-generated pages into the wiki + maintain aggregates + cache. */
export async function commitPages(kb, pages, sourceFiles, options) {
    if (pages.length === 0)
        throw new Error('no pages to commit');
    const created = [];
    const updated = [];
    const now = today();
    const existingByPath = new Map();
    for (const card of await listCards(kb))
        existingByPath.set(card.path, card);
    for (const page of pages) {
        const title = page.title.trim();
        if (title === '')
            throw new Error('every page needs a title');
        const type = page.type.trim();
        if (type === '')
            throw new Error(`page "${title}" needs a type`);
        ...
        if (page.frontmatter !== undefined) {
            for (const [key, value] of Object.entries(page.frontmatter)) {
                if (!isManagedFrontmatterKey(key) && value !== undefined && value !== null)
                    fm[key] = value;
            }
        }
    ...
    if (sourceFiles.length > 0)
        notes.push(`资料源: ${sourceFiles.join('、')}`);
    if (options?.extraNotes !== undefined)
 ...
 */
export async function createCard(kb, input) {
    ...
    if (card === null)
        throw new Error(`卡片创建失败: ${input.title}`);
...
export async function listSources(kb) {
...
export async function pendingSources(kb) {
    const sources = await listSources(kb);
...
// summaries
// ---------------------------------------------------------------------------
export async function kbSummary(kb) {
    const cards = await listCards(kb);
    const sources = await listSources(kb);
    const byType = {};
    let maxUpdated;
...
export async function listKbSummaries() {
...
export async function rebuildAggregates(kb) {
    await rebuildIndex(kb);
    await rebuildOverview(kb);
 ...
 */
export async function importCards(kb, dir) {
    ...
    if (pages.length === 0)
        return { imported: [], skipped, sourceFiles };
    const extraNotes = [];
    if (skipped.length > 0)
        extraNotes.push(`跳过无效文件 ${skipped.length} 个: ${skipped.map((item) => item.file).join('、')}`);
 ...
 */
export async function editCard(kb, slug, input) {
    const existing = await readCard(kb, slug);
    if (existing === null)
        throw new Error(`卡片不存在: ${slug}`);
    ...
    if (input.frontmatterYaml !== undefined) {
        ...
        const comparable = (object) => JSON.stringify({ ...object, updated: '\u0000' });
    ...
    if (changed.length > 0) {
        ...
        const notes = [];
        if (input.extraNotes !== undefined && input.extraNotes.length > 0)
            notes.push(...input.extraNotes);
        if (fullFmReplaced)
            notes.push('frontmatter: 规则配置已更新（YAML 全量替换）');
        if (structuredFmReplaced)
            notes.push('frontmatter: 字段元数据已更新（结构化表单）');
        if (title !== undefined && title !== '' && title !== existing.title) {
...
export async function listCodeFiles(kb) {
...
export async function readCodeFile(kb, relPath) {
    const safe = safeCodeRelPath(relPath);
    if (safe === null)
        throw new Error(`invalid code path: ${relPath}`);
    return fs.readFile(join(codeDir(kb), safe), 'utf8');
}
/** Write one code file (UTF-8, raw content — no frontmatter, no LLM). */
export async function writeCodeFile(kb, relPath, content) {
    const safe = safeCodeRelPath(relPath);
    if (safe === null)
        throw new Error(`invalid code path: ${relPath}`);
...
export async function deleteCodeFile(kb, relPath) {
    const safe = safeCodeRelPath(relPath);
    if (safe === null)
        throw new Error(`invalid code path: ${relPath}`);
    const full = join(codeDir(kb), safe);
...
export async function listTrashedCards(kb) {
...
async function findTrashedCard(kb, slug) {
    const target = slug.replace(/^wiki\//, '').replace(/\.md$/, '');
...
export async function deleteCard(kb, slug) {
    const card = await readCard(kb, slug);
    if (card === null)
        throw new Error(`卡片不存在: ${slug}`);
...
export async function restoreCard(kb, slug) {
    const entry = await findTrashedCard(kb, slug);
    if (entry === null)
        throw new Error(`回收站中没有该卡片: ${slug}`);
...
export async function purgeCard(kb, slug) {
    const entry = await findTrashedCard(kb, slug);
    if (entry === null)
        throw new Error(`回收站中没有该卡片: ${slug}`);
...
async function readTrashMeta(trashDir) {
...
export async function listTrashedKbs() {
...
async function findTrashedKb(id) {
...
export async function deleteKb(id) {
    ...
    if (kb === undefined)
        throw new Error(`知识库不存在: ${id}`);
    let trashDir = join(kbTrashRoot(), kb.id);
    if (await pathExists(trashDir))
        trashDir = join(kbTrashRoot(), `${kb.id}-${Date.now()}`);
...
export async function restoreKb(id) {
    const trashDir = await findTrashedKb(id);
    if (trashDir === null)
        throw new Error(`回收站中没有该知识库: ${id}`);
    const meta = await readTrashMeta(trashDir);
    const config = await loadConfig();
    if (config.kbs.some((kb) => kb.id === id))
        throw new Error(`恢复冲突: 知识库 id "${id}" 已被占用`);
    const target = meta !== null && meta.originalPath !== '' ? meta.originalPath : join(configRoot(), 'kbs', id);
    if (await pathExists(target))
        throw new Error(`恢复冲突: 原路径已存在: ${target}`);
    ...
    if (await pathExists(reviewsBackup)) {
        ...
        await movePath(reviewsBackup, reviewsFile({ id }));
...
export async function purgeKb(id) {
    const trashDir = await findTrashedKb(id);
    if (trashDir === null)
        throw new Error(`回收站中没有该知识库: ${id}`);
...
export async function addReview(kb, input) {
...
export async function listReviews(kb, status) {
    const items = await loadReviews(kb);
...
export async function resolveReview(kb, id, status, resolution) {
    const items = await loadReviews(kb);
