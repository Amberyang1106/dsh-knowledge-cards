...
import { RULE_OPERATORS, RULE_STATUSES } from "../core/types.js";
import { listCards, readCard, today } from "./store.js";
...
function inEffectWindow(effectiveFrom, effectiveTo) {
    ...
    if (effectiveFrom !== undefined && effectiveFrom > now)
        return false;
    if (effectiveTo !== undefined && effectiveTo < now)
        return false;
    return true;
}
...
function validateSpec(fm) {
    const issues = [];
    if (asString(fm.rule_id) === undefined)
        issues.push('缺 rule_id');
    const ruleSet = asString(fm.rule_set);
    if (ruleSet === undefined)
        issues.push('缺 rule_set');
    const status = asString(fm.status) ?? 'draft';
    if (!RULE_STATUSES.includes(status))
        issues.push(`status 非法: ${status}（应为 ${RULE_STATUSES.join('|')}）`);
    const conditions = Array.isArray(fm.conditions) ? fm.conditions : [];
    if (conditions.length === 0)
        issues.push('conditions 为空：至少一条 {fact, operator, value} 条件');
    conditions.forEach((entry, index) => {
        ...
        if (asString(condition.fact) === undefined)
            issues.push(`${prefix} 缺 fact`);
        const operator = asString(condition.operator);
        if (operator === undefined) {
            issues.push(`${prefix} 缺 operator`);
        }
        else if (!RULE_OPERATORS.includes(operator)) {
    ...
    else if (asString(outcome.category) === undefined) {
 ...
 */
export async function compileRuleSet(kb, opts = {}) {
    ...
    const wantedSet = (candidate) => ruleSet === undefined || candidate === ruleSet;
    const compiled = [];
    const invalid = [];
    ...
    for (const meta of cards) {
        ...
        if (!wantedSet(asString(fm.rule_set)))
            continue;
