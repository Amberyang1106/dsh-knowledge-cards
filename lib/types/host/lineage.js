...
import { configRoot, editCard, listCards, readCard } from "./store.js";
...
function identifiersOf(view) {
    ...
    if (Array.isArray(view.fm.aliases))
        raw.push(...view.fm.aliases);
 ...
 */
export async function scanLineage(kb) {
    ...
    for (const card of cards) {
        for (const identifier of identifiersOf(card)) {
            ...
            if (!byIdentifier.has(key))
                byIdentifier.set(key, card.meta.slug);
        }
    }
    ...
    for (const card of cards) {
        ...
        for (const other of cards) {
            if (other.meta.slug === card.meta.slug)
                continue;
...
export function lineageProposalFile(kb) {
    return join(configRoot(), 'lineage', `${kb.id}.json`);
}
export async function readLineageProposals(kb) {
    try {
...
export async function writeLineageProposals(kb, proposals, extra = {}) {
    const file = {
        kb: kb.id,
 ...
 */
export function buildLineagePrompt(kb, cards) {
...
export async function listFieldCardMeta(kb) {
 ...
 */
export async function listFieldCardScopes(kb) {
 ...
 */
export async function confirmFieldCards(kb, slugs) {
    const confirmed = [];
    const skipped = [];
    for (const slug of [...new Set(slugs)]) {
        ...
        for (const [key, value] of Object.entries(current)) {
            if (['type', 'title', 'description', 'tags', 'related', 'sources', 'created', 'updated'].includes(key))
                continue;
            next[key] = value;
        }
 ...
 */
export async function applyLineage(kb, accepted) {
    const applied = [];
    const skipped = [];
    ...
    for (const [slug, proposals] of bySlug) {
        ...
        const relations = {};
        for (const key of RELATION_KEYS)
            relations[key] = asStringList(fm[key]);
