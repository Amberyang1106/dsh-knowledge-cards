...
export function credentialLookup(ctx) {
 ...
 */
export async function resolveJevConfig(env = process.env, lookup) {
    const candidates = [
...
export async function buildJevCardScopes(kb, excerptChars = LINEAGE_CONFIG_DEFAULTS.excerptChars) {
    const scopes = [];
...
export async function buildJevState(kb, excerptChars) {
 ...
 */
export function buildJevQuestions(states, options = {}) {
    ...
    for (const source of states) {
        ...
        for (const target of states) {
            if (source.slug === target.slug)
                continue;
            if (!sourceJudged && !isJudged(target.slug))
                continue;
 ...
 */
export function planJevScope(scopes, params, force = {}) {
 ...
 */
export function jevProposals(kb, states, answers, options = PROPOSAL_DEFAULTS) {
 ...
 */
export async function runJevLineage(kb, transport, options = {}) {
    const resolved = transport ?? (await resolveJevConfig());
    if (resolved === null) {
        throw new Error('未配置 JEV key：请在 DSH 凭据库（~/.dsh/.credentials.yaml）或环境变量设置 OPENROUTER_API_KEY（默认线路）或 TYPESAFE_API_KEY');
    }
    ...
    if (plan.judgedCardCount === 0) {
        throw new Error(`全部 ${scopes.length} 张字段卡都已确认（review_status=confirmed），本轮无卡可判；如需重判请在面板勾选「强制重判」或打开「全量重跑」`);
    }
    if (plan.judgedCardCount > params.maxCards) {
        throw new Error(`本轮参与判定的字段卡 ${plan.judgedCardCount} 张（共 ${scopes.length} 张，已跳过 ${plan.skipped.length} 张已确认卡），超过上限 ${params.maxCards} 张；请先确认更多卡片把它们跳过，或在面板里调高 maxCards`);
    }
    if (plan.questionCount > params.maxQuestions) {
        throw new Error(`本轮问题数 ${plan.questionCount} 超过上限 ${params.maxQuestions}；请先确认更多卡片（跳过已维护的）、减少参与判定的卡片，或在面板里调高 maxQuestions`);
    }
    ...
    for (const batch of batches) {
        ...
        if (batchChars > params.payloadBudgetChars) {
            throw new Error(`JEV 单批请求体 ${batchChars} 字符，超过预算 ${params.payloadBudgetChars}（32k 上下文的保守估计；本轮 state 含 ${plan.sent.length} 张卡、每批 ${Object.keys(batch).length} 个问题）；请调小 excerptChars/questionsPerRequest，或减少参与判定的卡片后重试`);
        }
