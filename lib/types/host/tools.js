...
import { lineageProposalFile, writeLineageProposals } from "./lineage.js";
import { auditKb } from "./audit.js";
import { addReview, commitPages, createKb, deleteCard, deleteKb, editCard, getKb, importCards, kbSummary, listCards, listCodeFiles, listKbSummaries, listReviews, listSources, listTrash, pendingSources, purgeCard, purgeKb, readCard, readCodeFile, restoreCard, restoreKb, } from "./store.js";
function text(value) {
        ...
        output: {
            ...
            render: (_args, value) => {
                if (value.kbs.length === 0)
                    return text('尚无知识库。可在 GUI 侧边栏「知识卡片」面板的「知识库」标签创建，或告诉 agent 创建。');
...
export function wikiSearchTool() {
    return defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                if (value.cards.length === 0)
                    return text(`知识库「${value.kb}」没有匹配的卡片（命中 ${value.total}）。可尝试换检索词，或 wiki_ingest 摄入相关资料。`);
        ...
        async execute(args) {
            ...
            if (resolved === null)
                throw new Error('尚无知识库。先创建知识库（GUI 面板「知识库」标签，或让 agent 用 wiki 工具创建）。');
            const kb = await getKb(resolved.id);
            if (kb === null)
                throw new Error(`unknown knowledge base: ${resolved.id}`);
...
export function wikiReadTool() {
    return defineTool({
        ...
        async execute(args) {
            const resolved = await resolveKb(args.kb);
            if (resolved === null)
                throw new Error('尚无知识库。先创建知识库。');
            const kb = await getKb(resolved.id);
            if (kb === null)
                throw new Error(`unknown knowledge base: ${resolved.id}`);
            const card = await readCard(kb, args.slug);
            if (card === null)
                throw new Error(`卡片不存在: ${args.slug}（先 wiki_search 确认 slug）`);
            return {
                kb: resolved.id,
                slug: args.slug,
                card: {
...
export function wikiCommitTool() {
    return defineTool({
        ...
        async execute(args) {
            ...
            if (args.pages.length > 20)
                throw new Error('单次 commit 最多 20 张卡片。');
    ...
    return defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                for (const path of value.imported)
                    lines.push(`  ✓ ${path}`);
        ...
        async execute(args) {
            const resolved = await resolveKb(args.kb);
            if (resolved === null)
                throw new Error('尚无知识库。先 wiki_create_kb 建库。');
            const kb = await getKb(resolved.id);
            if (kb === null)
                throw new Error(`unknown knowledge base: ${resolved.id}`);
            if (typeof args.dir !== 'string' || args.dir.trim() === '')
                throw new Error('dir 必填：要导入的卡片目录绝对路径。');
            return {
                kb: resolved.id,
        ...
        output: {
            ...
            render: (_args, value) => {
                if (value.files.length === 0)
                    return text(`知识库「${value.kb}」暂无代码文件。可在侧边栏「知识卡片」→「代码」标签上传。`);
    ...
    return defineTool({
        ...
        async execute(args) {
            ...
            if (!Array.isArray(args.proposals) || args.proposals.length === 0)
                throw new Error('proposals 不能为空。');
            const RELATION_KEYS = ['depends_on', 'used_by', 'implemented_in', 'governed_by'];
        ...
        output: {
            ...
            render: (_args, value) => {
                if (value.items.length === 0)
                    return text(`知识库「${value.kb}」审核队列为空。`);
    ...
    return defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                for (const item of value.submitted)
...
export function wikiKbDeleteTool() {
    return defineTool({
        ...
        async execute(args) {
            if (typeof args.kb !== 'string' || args.kb.trim() === '')
                throw new Error('kb 必填：要删除的知识库 id。');
...
export function wikiKbRestoreTool() {
    return defineTool({
        ...
        async execute(args) {
            if (typeof args.kb !== 'string' || args.kb.trim() === '')
                throw new Error('kb 必填：回收站中的知识库 id。');
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                if (value.kbs.length > 0) {
                    ...
                    for (const kb of value.kbs)
                        lines.push(`- [kb] ${kb.id} | ${kb.name} | 原路径: ${kb.originalPath} | 删除于 ${new Date(kb.deletedAt).toLocaleString()}`);
                }
                if (value.cards.length > 0) {
                    lines.push(`回收站中已删除的卡片 ${value.cards.length} 张:`);
                    for (const card of value.cards)
                        lines.push(`- [${card.type}] ${card.slug} — ${card.title} | 原路径: ${card.originalPath} | 删除于 ${new Date(card.deletedAt).toLocaleString()}`);
                }
                if (lines.length === 0)
                    lines.push('回收站为空。');
