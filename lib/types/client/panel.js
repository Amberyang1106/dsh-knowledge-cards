...
const TYPE_FILTERS = ['all', 'entity', 'concept', 'source', 'query', 'comparison', 'synthesis', 'rules'];
/** Types offered by the manual create form (overview is auto-maintained). */
const CREATE_TYPES = ['concept', 'entity', 'source', 'query', 'comparison', 'synthesis', 'rules'];
...
function CardsTab({ kbId, onOpenCard, }) {
    ...
    const saveCreate = async () => {
        if (isRuleDraft ? draftYaml.trim() === '' : draftTitle.trim() === '')
            return;
        setSavingDraft(true);
        setCreateError(null);
        try {
