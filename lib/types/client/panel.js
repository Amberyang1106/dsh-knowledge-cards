    ...
    const switchDraftType = (type) => {
        ...
        if (draftYaml.trim() === '') {
            if (type === 'rules')
                ...
                setDraftYaml(FIELD_YAML_TEMPLATE);
        }
    };
    const saveCreate = async () => {
        if (isYamlDraft ? draftYaml.trim() === '' : draftTitle.trim() === '')
            return;
        setSavingDraft(true);
        setCreateError(null);
        try {
