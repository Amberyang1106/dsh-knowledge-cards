...
declare const zh: {
    ...
    readonly 'lineage.jevDone': "JEV 完成：{n} 条候选（{line} · key 来源 {keySource} · model {model} · 参与判定 {cards} 张卡、跳过 {skipped} 张已确认卡 · {questions} 个问题 · {requests} 批请求，最大 {payload} 字符 · 参数 {fp}）";
    readonly 'lineage.jevKeyMissing': "未配置 JEV key：写入 DSH 凭据库 ~/.dsh/.credentials.yaml（推荐，优先级最高、改完立即生效、无需重启），或设环境变量 OPENROUTER_API_KEY（openrouter.ai/keys）／TYPESAFE_API_KEY（console.typesafe.ai/keys）。插件只读凭据、只发最小化 state（元数据 + 去代码/去数字摘要）：卡片内容在 state 里各出现一次、问题仅按 slug 引用。";
    ...
    readonly 'lineage.scopeTitle': "🎯 判定范围（跳过已确认卡）";
    readonly 'lineage.scopeSummary': "字段卡共 {total} 张，已确认 {confirmed} 张；本轮实际参与判定 {judged} 张（已确认的卡默认跳过，但新增卡与老卡之间的边仍会被判）。";
    readonly 'lineage.scopeForceAll': "全量重跑";
    readonly 'lineage.scopeForceAllOff': "关闭（按已确认跳过）";
    readonly 'lineage.scopeForceAllOn': "开启（忽略跳过、全库重判）";
    readonly 'lineage.scopeColForce': "强制重判";
    readonly 'lineage.scopeColCard': "字段卡";
    readonly 'lineage.scopeColStatus': "review_status";
    readonly 'lineage.scopeColLineage': "已有血缘";
    readonly 'lineage.scopeColConfirm': "标记已确认";
    readonly 'lineage.scopeForceHint': "勾选后本卡即使已确认也会参与本轮判定（字段逻辑更新后用它补新增的血缘）";
    readonly 'lineage.scopeStatusEmpty': "（未填）";
    readonly 'lineage.scopeConfirmed': "已确认";
    readonly 'lineage.scopeConfirm': "标记为已确认（{n}）";
    readonly 'lineage.scopeConfirmHint': "确认后该卡默认不再被 JEV 判定；只改 review_status，不动 evidence_level。";
    readonly 'lineage.scopeConfirmedDone': "已确认 {n} 张（跳过 {skipped} 张）。";
    readonly 'lineage.paramsTitle': "⚙️ 判定参数";
    readonly 'lineage.paramsChangedCount': "已改 {n} 项";
    readonly 'lineage.paramsHint': "改动先存在草稿里，点「保存」才写盘（保存后下一轮生效，无需重启）。标「影响判定结果」的参数会改变产出本身，其余只影响显示或体量。";
    readonly 'lineage.paramsFileIssues': "配置文件有问题（已回退默认值运行）：";
    readonly 'lineage.paramChanged': "默认 {value}";
    readonly 'lineage.paramsSave': "保存参数";
    readonly 'lineage.paramsReset': "恢复默认（需保存）";
    readonly 'lineage.paramsMeta': "参数指纹 {fp} · 文件 {path}";
    readonly 'lineage.paramsSaved': "参数已保存（指纹 {fp}），下一轮判定即生效。";
    readonly 'lineage.paramsInvalid': "参数校验未通过：请按各项提示修正后重试（非法值不会被写盘）。";
    readonly 'param.confidenceHigh': "confidenceHigh｜high 档阈值（影响显示）";
    readonly 'param.confidenceMedium': "confidenceMedium｜medium 档阈值（影响显示）";
    readonly 'param.depThreshold': "depThreshold｜低于此值不出边（影响判定结果）";
    readonly 'param.additiveThreshold': "additiveThreshold｜可加性判定阈值（影响判定结果）";
    readonly 'param.askFieldKind': "askFieldKind｜是否问 field_kind";
    readonly 'param.askAdditive': "askAdditive｜是否问可加性";
    readonly 'param.excerptChars': "excerptChars｜每卡正文摘要长度";
    readonly 'param.maxCards': "maxCards｜本轮参与判定的卡片上限";
    readonly 'param.maxQuestions': "maxQuestions｜单轮问题总数上限";
    readonly 'param.questionsPerRequest': "questionsPerRequest｜每批问题数";
    readonly 'param.payloadBudgetChars': "payloadBudgetChars｜单批请求体预算（字符）";
    readonly 'param.skipConfirmed': "skipConfirmed｜跳过双方都已确认的卡片对";
