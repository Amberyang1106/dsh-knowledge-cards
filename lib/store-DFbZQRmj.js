...
async function commitPages(kb, pages, sourceFiles, options) {
	...
	for (const page of pages) {
		...
		if (page.frontmatter !== void 0) {
			for (const [key, value] of Object.entries(page.frontmatter)) if (!isManagedFrontmatterKey(key) && value !== void 0 && value !== null) fm[key] = value;
		}
...
*/
async function createCard(kb, input) {
	const result = await commitPages(kb, [input], input.sources ?? [], { logAction: "create" });
	const card = await readCard(kb, slugFromTitle(input.title));
...
async function editCard(kb, slug, input) {
	...
	if (input.frontmatterYaml !== void 0) {
		const parsedFull = parseYamlPayload(input.frontmatterYaml);
		if (parsedFull === null || typeof parsedFull.title !== "string" || parsedFull.title.trim() === "") throw new Error("YAML 无效：必须包含 title（非空）。type 保持不变，无需重复提供也可保留。");
		if (parsedFull.type !== void 0 && parsedFull.type !== existing.type) throw new Error(`type 不可更改（保持 ${existing.type}）`);
		const fresh = {};
		...
		fresh.created = existing.created ?? (typeof fresh.created === "string" && fresh.created !== "" ? fresh.created : void 0) ?? now;
		delete fresh.updated;
		const comparable = (object) => JSON.stringify({
			...object,
			updated: "\0"
		});
	...
	if (input.frontmatter !== void 0) {
		...
		const current = {};
		for (const [key, value] of Object.entries(fm)) if (!isManagedFrontmatterKey(key)) current[key] = value;
		if (JSON.stringify(current) !== JSON.stringify(next)) {
			changed.push("字段元数据");
			structuredFmReplaced = true;
		}
		for (const key of Object.keys(fm)) if (!isManagedFrontmatterKey(key)) delete fm[key];
	...
	if (changed.length > 0) {
		...
		const notes = [];
		if (input.extraNotes !== void 0 && input.extraNotes.length > 0) notes.push(...input.extraNotes);
...
async function listTrashedCards(kb) {
...
async function findTrashedCard(kb, slug) {
	const target = slug.replace(/^wiki\//, "").replace(/\.md$/, "");
	return (await listTrashedCards(kb)).find((entry) => entry.slug === target || slugFromTitle(entry.title) === target) ?? null;
}
/** Soft-delete one card: move its file into the KB recycle bin, rebuild
* index/overview and append a `delete` log entry. */
async function deleteCard(kb, slug) {
	...
	await promises.mkdir(dirname(trashPath), { recursive: true });
...
async function restoreCard(kb, slug) {
	...
	if (await pathExists(target)) throw new Error(`恢复冲突: wiki/${entry.originalPath} 已存在（可能已重建同 slug 卡片）。请先彻底删除回收站中的旧条目，或手动处理。`);
...
async function purgeCard(kb, slug) {
	const entry = await findTrashedCard(kb, slug);
	if (entry === null) throw new Error(`回收站中没有该卡片: ${slug}`);
	await promises.rm(entry.trashPath, { force: true });
...
async function listTrashedKbs() {
	const root = kbTrashRoot();
	let dirs;
	try {
		dirs = await promises.readdir(root);
	} catch {
		return [];
	}
	const entries = [];
	for (const dir of dirs) {
		...
		let stat;
		try {
			stat = await promises.stat(trashDir);
		} catch {
			continue;
		}
		entries.push({
...
async function findTrashedKb(id) {
	...
	return (await listTrashedKbs()).find((entry) => entry.id === id)?.trashPath ?? null;
}
...
async function deleteKb(id) {
	...
	if (await pathExists(reviewsSrc)) {
		await promises.mkdir(join(trashDir, "reviews"), { recursive: true });
...
async function restoreKb(id) {
	...
	if (await pathExists(reviewsBackup)) {
		await promises.mkdir(join(configRoot(), "reviews"), { recursive: true });
		await movePath(reviewsBackup, reviewsFile({ id }));
		await promises.rm(join(target, "reviews"), {
			recursive: true,
			force: true
		});
	}
...
async function purgeKb(id) {
	const trashDir = await findTrashedKb(id);
	if (trashDir === null) throw new Error(`回收站中没有该知识库: ${id}`);
	await promises.rm(trashDir, {
		recursive: true,
		force: true
	});
	return { purged: true };
...
//#endregion
export { rebuildAggregates as A, listTrashedCards as C, purgeKb as D, purgeCard as E, sha256Of as F, today as I, writeCodeFile as L, resolveReview as M, restoreCard as N, readCard as O, restoreKb as P, RULE_OPERATORS as R, listTrash as S, pendingSources as T, listKbSummaries as _, createKb as a, listReviews as b, deleteCodeFile as c, ensureKbStructure as d, getKb as f, listCodeFiles as g, listCards as h, createCard as i, resolveLinkTargets as j, readCodeFile as k, deleteKb as l, kbSummary as m, commitPages as n, defaultKbId as o, importCards as p, configRoot as r, deleteCard as s, addReview as t, editCard as u, listKbs as v, listTrashedKbs as w, listSources as x, listLogEntries as y, RULE_STATUSES as z };
