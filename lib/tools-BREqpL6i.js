...
function wikiLineageProposeTool() {
	return defineTool({
		...
		parameters: {
				...
				items: {
					...
					properties: {
						slug: {
							type: "string",
							required: true,
							description: "要更新的字段卡 slug。"
						},
						title: {
							type: "string",
							description: "卡片标题（展示用）。"
						},
						confidence: {
							type: "string",
						...
						metadata: {
							type: "object",
							additionalProperties: true,
							description: "元数据修正建议（空串=删除该键）。"
						},
						note: {
							type: "string",
							description: "可选备注（如目标卡缺失）。"
						}
					}
				}
			},
			notes: {
				type: "array",
				items: { type: "string" },
				description: "本次分析的总体说明（可含无法建边的观察）。"
			}
		},
		output: {
			schema: {
		...
		async execute(args) {
			...
			const proposals = args.proposals.map((item) => {
				...
				for (const key of RELATION_KEYS) {
					...
					const list = Array.isArray(value) ? value.map((entry) => String(entry).trim()).filter((entry) => entry !== "") : String(value).split(/[\n,]/).map((entry) => entry.trim()).filter((entry) => entry !== "");
				...
				return {
					...
					title: item.title !== void 0 ? String(item.title) : String(item.slug),
					source: "llm",
					confidence,
...
//#endregion
export { applyLineage as C, scanLineage as D, readLineageProposals as E, wikiTrashListTool as S, listFieldCardMeta as T, wikiLintTool as _, wikiCodeListTool as a, wikiReviewsTool as b, wikiCreateKbTool as c, wikiIngestTool as d, wikiKbDeleteTool as f, wikiLineageProposeTool as g, wikiKbsTool as h, wikiCardRestoreTool as i, wikiEditCardTool as l, wikiKbRestoreTool as m, wikiCardDeleteTool as n, wikiCodeReadTool as o, wikiKbPurgeTool as p, wikiCardPurgeTool as r, wikiCommitTool as s, wikiAuditTool as t, wikiImportCardsTool as u, wikiReadTool as v, buildLineagePrompt as w, wikiSearchTool as x, wikiReviewSubmitTool as y };
