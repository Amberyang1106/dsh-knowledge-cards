...
*/
function resolveJevConfig(env = process.env) {
...
*/
async function runJevLineage(kb, config) {
	const resolved = config ?? resolveJevConfig();
	if (resolved === null) throw new Error("未配置 JEV key：请设置 OPENROUTER_API_KEY（默认线路）或 TYPESAFE_API_KEY 后重启 dsh web");
	const states = await buildJevState(kb);
	if (states.length === 0) throw new Error("该知识库没有字段卡（type=field）");
	if (states.length > MAX_CARDS) throw new Error(`字段卡 ${states.length} 张，超过 JEV 单轮上限 ${MAX_CARDS} 张（问题数随卡片数平方增长）；请按 subject_area 分批，或先用确定性预扫`);
	...
	for (const batch of batches) {
		...
		if (batchChars > MAX_PAYLOAD_CHARS) throw new Error(`JEV 单批请求体 ${batchChars} 字符，超过预算 ${MAX_PAYLOAD_CHARS}（32k 上下文的保守估计；本轮字段卡 ${states.length} 张、每批 ${Object.keys(batch).length} 个问题）；请减少字段卡数量后重试`);
...
//#endregion
export { buildJevQuestions, buildJevState, callJev, jevProposals, resolveJevConfig, runJevLineage };
