...
## v0.6.0（2026-09-22）— 参数可视化编辑 + 跳过已确认卡（判定范围）

### ✨ 新功能 1：⚙️ 判定参数可视化编辑（保存后生效）

- 12 个此前硬编码的参数全部可配：阈值 4 项（`confidenceHigh` / `confidenceMedium` / `depThreshold` / `additiveThreshold`）、开关 3 项（`askFieldKind` / `askAdditive` / `skipConfirmed`）、体量 5 项（`excerptChars` / `maxCards` / `maxQuestions` / `questionsPerRequest` / `payloadBudgetChars`）。
- **界面**：血缘补齐面板新增「⚙️ 判定参数」折叠区，控件由**服务端下发的字段描述**渲染（取值范围单一来源，客户端不重复定义）。改动先存草稿，点「保存」才写盘——保存后下一轮生效、**无需重启**（配置按请求读取）。「影响判定结果」与「仅影响显示」的参数在标签上区分。
- **校验**：保存时逐字段校验并**拒绝写入**（非法值不落盘）；文件被外部改坏时**大声报错并回退默认值**运行，面板显示具体问题。跨字段规则：`confidenceHigh` 必须大于 `confidenceMedium`。
- **只写差异**：配置文件 `<configRoot>/lineage/<kbId>.config.json` 只保存与默认值不同的键（全默认时是 `{}`），因此后续版本改默认值能自动惠及未覆盖过该键的库。
- **可追溯**：参数指纹（12 位）出现在面板状态行、每轮结果与**每条 JEV 提案的 note** 里，应用时还会写进看板日志——日后看到一条奇怪的边，能查出当时用的是哪套参数。

### ✨ 新功能 2：🎯 判定范围——跳过已确认卡（配对级）

- **规则**：只跳过**双方都已 `review_status: confirmed`** 的**有序卡片对**。这是关键设计——按卡片整张跳过会永久漏掉「新卡 × 老卡」之间的新边，而那正是知识库增长时最需要补的边。
- **面板**：新增「🎯 判定范围」折叠区，显示「共 N 张 / 已确认 K 张 / 本轮参与 M 张」，逐卡列出 `review_status` 与已有血缘数量，并提供三个入口：① **强制重判**（勾选某张已确认卡，字段逻辑变更后用它补新增血缘）② **全量重跑**（忽略跳过、整库重判）③ **一键标记为已确认**（批量，只改 `review_status`、不动 `evidence_level`）。
- **上限口径修正**：`maxCards` 现在计算**参与判定的卡片数**（而非全库卡片数），并新增 `maxQuestions` 作为更直接的问题数闸。这样大库只要确认了大部分卡片就能真正跑起来——此前只要超过 20 张字段卡就**直接报错、完全跑不了**。
- **实测**（真实 isg-service 库，8 张字段卡、其中 2 张已 confirmed）：默认跳过 2 张、免 2 个配对，问题数 72 → 66、2 批请求完成；模拟 5 张已确认时降至 3 张参与、42 个问题、1 批；强制重判 1 张后回到 4 张参与、52 个问题。
- 全部卡片都已确认且未开启强制/全量时，返回明确提示（「无卡可判」）而不是空跑。

### 🔧 其他

- 新增路由 3 条：`GET|POST /lineage/config`（读写参数）、`GET /lineage/cards`（判定范围清单）、`POST /lineage/confirm`（批量标记已确认）→ 路由 32→35。
- `editCard` 支持 `extraNotes`，让血缘应用把「血缘来源: 参数指纹 …」写进看板日志。
- 测试 74/74（新增 18 个：参数校验/稀疏写入/指纹/坏文件回退、配对级跳过与强制重判、阈值生效、三个新路由的端到端、面板参数保存流程）。冒烟 79/79。

> ⚠️ **验证状态**：JEV 的**真实 API 调用仍未执行过**（本机无 key），上述实测均为打桩 fetch；参数阈值（0.8 / 0.5 / 0.5 / 0.2）依旧是**未校准的初值**——现在你可以直接在面板里调，无需改代码。
...
### ✨ 新功能 3：key 走 DSH 凭据服务（四层解析，改 key 无需重启）

- **背景**：DSH 有分层凭据/环境解析（继承环境 > `$DSH_HOME/.credentials.yaml` > 启动目录 `.env` > `$DSH_HOME/.env`），但托管凭据库的内容**不会进入 `process.env`**。原先按 `process.env` 取值，导致"Models 页面/凭据库里已有 OpenRouter key，插件却报未配置"。
- **做法**：改经 DSH 官方 **`ctx.credentials` 服务**解析（`OPENROUTER_API_KEY` → `TYPESAFE_API_KEY` 依次尝试），并在结果里回带 `keySource`（`env` / `file` / `project-env` / `user-env`），面板状态行显示"key 来源"，可见地证明读到了哪一层。
- **不引入新运行时依赖**：`credentialRef()` 只是运行时正则校验并原样返回变量名，故插件用鸭子接口访问服务；服务缺失（宿主未挂载凭据提供者）时自动回退 `process.env`，插件照常加载。已核实 `dsh-base` bundle 挂载了 `id: credentials`，web profile 下该服务存在。
- **无重启成本**：凭据服务按操作解析、不跨操作缓存，所以改 key 下一次点按钮即生效。
- `.gitignore` 增加 `.env` / `.env.*`（保留 `.env.example`），避免临时测试用的 key 被 `git add -A` 带上。
- 测试 56/56（新增：凭据层解析与来源标注、服务缺失/畸形时的容错）。
...
- 测试 56/56（新增 5 个：JSON 去重保证、线路解析优先级、分批与 usage 合并、凭据层解析与来源标注、服务缺失容错）。
